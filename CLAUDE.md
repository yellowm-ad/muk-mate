...
전북대 덕진구 생활권 학생들을 위한 공동주문 매칭 모바일 웹 서비스. 전체 요구사항·근거는 **`docs/PRD.md` (v2.8)가 단일 소스**다 — 이 파일은 방향을 잡기 위한 요약이며, 세부 규칙은 아래 스킬 문서를 따른다.
...
- **2026-08-10 매너 포만도·아바타 신설(v2.8, PRD §17-5)**: 공동주문 완료 후 호스트↔승인 참여자 상호평가(3단계: 좋았어요/보통/아쉬웠어요)로 "매너 포만도"(0~100점)를 산출하고, 6단계(새로운 메이트 포함) 아바타로 표현하는 경량 신뢰 지표 기능. PRD §12 비목표("고도화된 평점·등급·포인트·쿠폰 시스템")와 충돌하는 요청이었으나, 랭킹·결제·자동제재·AI판정을 스스로 배제한 경량 설계라 예외로 허용하는 쪽으로 §12 문구를 좁혔다(§17-5 참고). 이번 구현은 P0 범위(평가·권한검사·점수계산·신규유저상태·5단계아바타·마이페이지/프로필 표시)까지만이고, 아바타 색상·소품 커스터마이징과 전용 알림은 후속 과제로 남겨뒀다.
- **남은 것**: 모집글 수정 화면(보류 중), P1 기능(Phase 6 — 거리 표시, 분담 금액), 매너 포만도 P1/P2(아바타 커스터마이징, 전용 알림, 관리자 매너 이벤트 연동)
...
5. 위 네 가지와 직접 관련 없는 기능은 미룬다 — 특히 결제/송금, 실시간 위치, AI 추천/매칭, 네이티브 앱, 학교 인증은 명시적 비목표(PRD §12)다. 포인트·쿠폰·랭킹 등 고도화된 게이미피케이션도 비목표이나, 경량 매너 포만도·아바타(PRD §17-5)는 예외로 허용됐다.
