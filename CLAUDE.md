...
- **남은 것**: 매너 포만도 P2(커스텀 SVG 캐릭터, 채팅/참여자목록 배지 노출, 평가 요청 알림) — 전부 원 기획안·PRD §17-5에서 의도적으로 미룬 항목. 친구 기능도 PRD 본문에는 아직 문서화 안 됨(이 CLAUDE.md 요약이 현재로선 유일한 근거). `participations.decided_at`/`decided_by` 고아 컬럼 정리도 남음(바로 위 항목 참고).
