...
3. Vercel 프로젝트의 환경변수가 설정되어 있는지 확인한다 (`vercel env ls`). 최소 다음이 있어야 한다:
   ...
   - `NAVER_CLIENT_ID`, `NAVER_CLIENT_SECRET` — 절대 `NEXT_PUBLIC_` 접두사를 붙이지 않는다.
...
- 브라우저 네트워크 탭 기준으로 `NAVER_CLIENT_SECRET`이 클라이언트 응답에 노출되지 않는지 확인이 필요함을 사용자에게 상기시킨다 (자동 확인이 어려우면 수동 점검을 요청한다).
