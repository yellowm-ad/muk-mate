...
- **DB 연결 문자열, 카카오 REST API 키 등은 절대 클라이언트 번들에 포함시키지 않는다.** 서버 전용 환경변수(`NEXT_PUBLIC_` 접두사 없는 변수)로만 다룬다.
...
- 장소 검색(`/api/places/search`)은 `kakao-places` 에이전트와 경계를 나눈다 — 이 에이전트는 서버가 프록시로 구현되어야 한다는 요건만 지키고, 카카오 API 연동 세부는 `kakao-places`에 위임한다.
